...
public class ListActivity extends AppCompatActivity {
  ...
  private void updateIndexLabels(boolean sortOrder) {
    ...
    for(int i = 0; i < indexLabels.length; i++) {
      ...
      int buttonId = getResources().getIdentifier("list_button_" + i,
          "id", getPackageName());
  ...
  private void setIndexClickListeners(RecyclerView recyclerView) {
    ...
    for(int i = 0; i < indexLabelsQty; i++) {
      int buttonId = getResources().getIdentifier("list_button_" + i, "id",
          getPackageName());
      int buttonKey = i;
      findViewById(buttonId).setOnClickListener(v ->
          ListActivity.this.navBtnClick(buttonKey, recyclerView));
    }
  }
}
